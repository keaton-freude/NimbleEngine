...
Material::Material(string name) : _name(std::move(name)) {
}
...
static TextureUnit GetTextureUnitFromJson(const Value &texture_obj) {
	...
	if(texture_obj.HasMember("sampler_settings")) {
		...
		if(sampler_settings.HasMember("texture_wrap_mode_u")) {
			...
			settings.textureWrapModeU =
				SamplerSettingsUtil::TextureWrapModeFromString(sampler_settings["texture_wrap_mode_u"].GetString());
		}
		...
		if(sampler_settings.HasMember("texture_wrap_mode_v")) {
			...
			settings.textureWrapModeV =
				SamplerSettingsUtil::TextureWrapModeFromString(sampler_settings["texture_wrap_mode_v"].GetString());
		}
		...
		if(sampler_settings.HasMember("texture_mag_filter")) {
			...
			settings.textureMagFilter =
				SamplerSettingsUtil::TextureMagFilterFromString(sampler_settings["texture_mag_filter"].GetString());
		}
		...
		if(sampler_settings.HasMember("texture_min_filter")) {
			...
			settings.textureMinFilter =
				SamplerSettingsUtil::TextureMinFilterFromString(sampler_settings["texture_min_filter"].GetString());
		}
	}
...
void Material::LoadFromFile(const char *path) {
	...
	ASSERT(document.HasMember("material_type") && document["material_type"].IsString(),
		   "Material JSON is missing `material_type` field, or it is not a string.");
